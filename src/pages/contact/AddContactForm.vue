    ...
    <div class="bg-white shadow-2xl rounded-2xl w-full max-w-4xl p-8">
      ...
      <form @submit.prevent="handleSubmit" class="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div class="flex flex-col">
          ...
          <input v-model="form.firstName" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-black" required />
        </div>
        <div class="flex flex-col">
        ...
        <div class="flex flex-col">
          ...
          <input v-model="form.email1" type="email" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-black" />
        </div>
        <div class="flex flex-col">
          <label class="block text-sm font-medium text-gray-700 mb-1">Email 2</label>
          <input v-model="form.email2" type="email" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-black" />
        </div>
        <div class="flex flex-col md:col-span-2">
          <label class="block text-sm font-medium text-gray-700 mb-1">Website / URL</label>
          <input v-model="form.url" type="url" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-black" />
        </div>
        <div class="flex flex-col">
        ...
        <div class="md:col-span-2 flex justify-between mt-6">
          <button type="submit" class="px-6 py-2 text-sm bg-black text-white hover:bg-black rounded shadow" :disabled="isLoading">
...
export default {
  setup() {
    ...
    const fetchContact = async (id) => {
      try {
        const res = await fetch(`https://dp7.wetechnoids.com/api/contacts/${id}`);
        if (!res.ok) {
        ...
        if (data) {
          form.value = {
            ...
            displayName: data.contact_order_by || '',
            privateEntry: data.contact_private === 1,
            jobTitle: data.contact_job || '',
            ...
            postcode: data.contact_zip || '',
            phone1: data.contact_phone || '',
            phone2: data.contact_phone2 || '',
            fax: data.contact_fax || '',
            mobilePhone: data.contact_mobile || '',
            email1: data.contact_email || '',
            email2: data.contact_email2 || '',
            url: data.contact_url || '',
            jabber: data.contact_jabber || '',
            icq: data.contact_icq || '',
            aol: data.contact_aol || '',
            msn: data.contact_msn || '',
            yahoo: data.contact_yahoo || '',
            birthday: data.contact_birthday || '',
            notes: data.contact_notes || ''
          };
        }
      } catch (err) {
        errorMessage.value = err.message || 'Failed to load contact data';
    ...
    const fetchCompanies = async () => {
      try {
        const res = await fetch('https://dp7.wetechnoids.com/api/companies');
        const data = await res.json();
    ...
    watch(() => route.query.id, (newId) => {
      if (newId) {
        ...
        errorMessage.value = 'No contact ID provided for editing';
        showPopup.value = true;
        setTimeout(() => {
          showPopup.value = false;
          router.push('/contacts');
    ...
    const handleSubmit = async () => {
      if (!form.value.id) {
        errorMessage.value = 'Cannot update contact: No ID provided';
        showPopup.value = true;
      ...
      try {
        const res = await fetch(`https://dp7.wetechnoids.com/api/contacts/${form.value.id}`, {
          method: 'PUT',
          headers: {
        ...
        errorMessage.value = err.message || 'Failed to update contact';
  ...
  setup() {
    ...
    watch(() => route.query.id, (newId) => {
      if (newId) {
        ...
        // Reset form if no ID (new contact)
        form.value = {
          id: null,
          firstName: '', lastName: '', displayName: '', privateEntry: false,
          jobTitle: '', company: '', department: '', title: '', type: '',
    ...
    // Existing fetchCompanies and fetchDepartments functions remain the same
    // ...
    ...
    const handleSubmit = async () => {
      ...
      try {
        const url = form.value.id 
          ? `https://dp7.wetechnoids.com/api/contacts/${form.value.id}`
          : 'https://dp7.wetechnoids.com/api/contacts';
        ...
        successMessage.value = form.value.id 
        ...
        errorMessage.value = err.message || 
          (form.value.id ? 'Failed to update contact.' : 'Failed to add contact.');
        successMessage.value = '';
        showPopup.value = true;
